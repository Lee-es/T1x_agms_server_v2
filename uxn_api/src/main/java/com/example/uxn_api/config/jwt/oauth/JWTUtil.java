...
//    private static final long REFRESH_TIME = 2;
    ...
    public static String makeAuthToken(User user){
        ...
        String authority = user.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toList()).get(0); // ROLE_USER
        return JWT.create()
                .withHeader(headerParam)
                .withSubject(user.getEmail()) // user_id
                .withClaim("authority", authority)
                .withClaim("idx",user.getId()) // 회원 정보를 가져오기 위해서 index 값도 넣어준다.
...
//    }
    ...
    public static String makeRefreshToken(User  user){
        Map<String, Object> headerParam = new HashMap<>();
        headerParam.put("typ","JWT");
        String authority = user.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toList()).get(0);
        return JWT.create()
                .withHeader(headerParam)
                .withSubject(user.getEmail())
                .withClaim("authority", authority)
                .withClaim("idx",user.getId())
                .withClaim("user_name", user.getUsername())
                .withClaim("user_id", user.getEmail())
